...
fn main() {
    ...
    thread::scope(|s| {
        for tid in 0..nthreads {
            let thread_seed = rng.random::<u64>();
            thread::Builder::new()
                .name(format!("t-{tid}"))
                .spawn_scoped(s, move || {
                    log::info!("thread={tid}, thread seed={thread_seed}");
                    let mut rng = ChaCha20Rng::seed_from_u64(thread_seed);
                    ...
                    for i in 0..maxn {
                        ...
                        match coin {
                            ...
                            5 => {
                                let key = format!("key-{}", rng.random_range(0..16));
                                let cat = format!("cat-{}", rng.random_range(0..16));
                                let val = rng.random_range(1..101);
